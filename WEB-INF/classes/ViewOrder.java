...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("ppppppppppppppppppppppppppppppp"+username);
		...
		// 	// FileInputStream fileInputStream = new FileInputStream(new File(TOMCAT_HOME+"\\webapps\\HW_1\\PaymentDetails.txt"));
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("ViewOrder"))
		{
			...
			{	
				...
				System.out.println(orderId+"rrrrrrrrrrrrrrrrrr");
				pw.print("<input type='hidden' name='orderId' value='"+orderId+"'>");
				//get the order details from file
				try
				{
					orderPayments = MySqlDataStoreUtilities.selectrder(orderId);
					...
					System.out.println(orderPayments+"jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj");
					// FileInputStream fileInputStream = new FileInputStream(new File(TOMCAT_HOME+"\\webapps\\HW_1\\PaymentDetails.txt"));
					// ObjectInputStream objectInputStream = new ObjectInputStream(fileInputStream);	      
					// orderPayments = (HashMap)objectInputStream.readObject();
				...
				System.out.println(orderPayments+"klklklklklklklklk");
				int size=0;
				...
				// {
				// 	for(OrderPayment od:orderPayments)	
				// 	{
				// 		System.out.println(od.getUserName()+"qqqqqqqqqqqqqqqqqqqqqqqq");
				...
				// {	
					...
					pw.print("<th>productPrice:</th></tr>");
					for (OrPayment oi : orderPayments) 
					{
						pw.print("<tr>");			
						pw.print("<td><input type='radio' name='orderName' value='"+oi.getOrderName()+"'></td>");			
						pw.print("<td>"+oi.getOrderId()+".</td><td>"+oi.getCustomerName()+"</td><td>"+oi.getOrderName()+"</td><td>Price: "+oi.getOrderPrice()+"</td>");
		...
		{
			System.out.print("cancel working in view order");
			if(request.getParameter("orderName") != null)
			{
				...
				ArrayList<OrPayment> ListOrderPayment =new ArrayList<OrPayment>();
				...
				for (OrPayment oi : orderPayments) 
					{
						...
						System.out.println(oi.getOrderName().equals(orderName));
						HttpSession session = request.getSession();
						System.out.println(session.getAttribute("username").equals(username));
							if(oi.getOrderName().equals(orderName) && session.getAttribute("username").equals(username))
							{
								System.out.println("hellloooooooooo");
				...
				// orderPayments.removeAll(ListOrderPayment);
				...
				// try
				// {	
				// 	FileOutputStream fileOutputStream = new FileOutputStream(new File(TOMCAT_HOME+"\\webapps\\HW_1\\PaymentDetails.txt"));
