...
public class Pchblist extends HttpServlet {
  ...
  //   }
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       try{
				...
				if(x !=null){
					pchblist = MySqlDataStoreUtilities.getPcbh(x);
				}else if(zip != null){
					pchblist = MySqlDataStoreUtilities.getPcbh(zip);
					} 
HashMap<String, ArrayList<Review>> hm = MongoDBDataStoreUtilities.selectReview();
		...
		System.out.println(pchblist.size()+"choosukoni ra ra");
		if(pchblist.size()==0){
			...
			System.out.println("After catch"+pchblist);
HttpSession session = request.getSession(); 
		session.setAttribute("Reviews",flatArray);
        request.setAttribute("Foodslist",pchblist);
        request.getRequestDispatcher("pchb.jsp").forward(request,response);
    }
}